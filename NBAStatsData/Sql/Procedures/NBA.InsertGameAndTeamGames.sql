CREATE OR ALTER PROCEDURE NBA.InsertGameAndTeamGames
	@GameDate DATE,
...
VALUES (@GameDate, @HomeAbbr + N' v. ' + @AwayAbbr);

DECLARE
@HomeID INT = (SELECT T.TeamID FROM NBA.Team T WHERE T.[Name] = @HomeTeam),
@AwayID INT = (SELECT T.TeamID FROM NBA.Team T WHERE T.[Name] = @AwayTeam);

INSERT NBA.TeamGame (GameID, TeamID, TeamTypeID, Score)
VALUES (SCOPE_IDENTITY(), @HomeID, 1, @HomeScore),
		(SCOPE_IDENTITY(), @AwayID, 2, @AwayScore);
GO
