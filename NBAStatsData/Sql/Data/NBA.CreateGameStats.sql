...
SELECT G.GameID, P.PlayerID, 
	...
	END as TeamID, 
	T.Points, T.Rebounds, T.Assists, T.Blocks, T.Steals, T.Turnovers, CAST(T.[Minutes] as decimal) as [Minutes]
FROM NBA.TempData T
	INNER JOIN NBA.Player P ON P.[Name] = T.PlayerName
	INNER JOIN NBA.Game G ON G.[Date] = T.GameDate
		AND (G.Matchup LIKE CONCAT('%', T.TeamAbbrev, '%') AND G.Matchup LIKE CONCAT('%', T.OpponentTeamAbbrev, '%'))
